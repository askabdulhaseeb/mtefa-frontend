...
void main() {
  group('Authentication Flow Integration Tests', () {
    ...
    test(
      'Complete successful login flow from UI to data persistence',
      () async {
        ...
        expect(
          ...
          isA<DataSuccess<LoginResponseEntity>>(),
        );
        ...
        verify(
          mockSecureStorage.write(key: 'remember_me', value: 'true'),
        ).called(1);
      },
    );
    ...
    test('Login with saved credentials flow', () async {
      ...
      when(
        mockSecureStorage.read(key: 'remember_me'),
      ).thenAnswer((_) async => 'true');
      when(
        mockSecureStorage.read(key: 'saved_email'),
      ).thenAnswer((_) async => AuthFixtures.validEmail);
      when(
        mockSecureStorage.read(key: 'saved_password'),
      ).thenAnswer((_) async => AuthFixtures.validPassword);

      // Act - Get saved credentials and login
      final Map<String, String>? savedCredentials = await authRepository
          .getSavedCredentials();
      ...
      expect(
        ...
        equals(AuthFixtures.validEmail),
      );
      expect(
        loginProvider.passwordController.text,
        equals(AuthFixtures.validPassword),
      );
    ...
    test('Complete logout flow', () async {
      ...
      // Act - Perform logout
      final DataState<void> logoutResult = await authRepository.logout();
    ...
    test('Get current user flow after successful login', () async {
      ...
      when(
        mockSecureStorage.read(key: 'current_user'),
      ).thenAnswer((_) async => '$userJson');

      // Act - Get current user
      final DataState<UserEntity> currentUserResult = await authRepository
          .getCurrentUser();
    ...
    test('Business switching flow', () async {
      ...
      // Act - Switch business
      const String newBusinessId = 'new_business_456';
      final DataState<UserEntity> switchResult = await authRepository
          .switchBusiness(businessId: newBusinessId);
      ...
      verify(
        mockSecureStorage.write(key: 'current_user', value: anyNamed('value')),
      ).called(1);
    });
    ...
    test('Error recovery flow - network failure during login', () async {
      ...
      final DataFailed failedState = loginProvider.loginState as DataFailed;
    ...
    test('Session refresh flow', () async {
      ...
      // Act
      final DataState<AuthTokenEntity> refreshResult = await authRepository
          .refreshToken(refreshToken: 'old_refresh_token');

      // Assert
      expect(refreshResult, isA<DataFailed<AuthTokenEntity>>());
      expect(
        (refreshResult as DataFailed).errorCode,
    ...
    test('Form validation and error clearing flow', () async {
      ...
      // Act - Validate again
      final bool isValidNow = loginProvider.validateForm();
